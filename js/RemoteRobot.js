define((require, exports, module) => {
  ...
  store.action("CHANGE_SEND_DATA", (state, data) =>
    Object.assign({}, state, {
      sendData: data,
    })
  );

  store.action("CHANGE_SEND_DATA_INTERVAL", (state, data) =>
    Object.assign({}, state, {
      sendDataInterval: data,
    })
  );

  store.action("SET_AVAILABLE_USB_DEVICES", (state, data) =>
    Object.assign({}, state, {
      USBDevices: data,
    })
  );

  store.action("SELECT_USB_DEVICE", (state, data) =>
    Object.assign({}, state, {
      selectedUSBDevice: data,
    })
  );

  store.action("CHANGE_REMOTE_STATUS", (state, data) =>
    Object.assign({}, state, {
      remoteStatus: data,
    })
  );
  ...
  store.listen((state) => {
    ...
    if (state.USBDevices[0] && !addedDropdownGui) {
      ...
      remoteRobotGui
        .add(guiHelper, "selectedUSBDevice", guiHelper.USBDevices)
        .onChange(() => {
    ...
    if (store.getState().remoteStatus === "portOpen") {
      ...
      if (store.getState().sendData) {
        interval = setInterval(() => {
          const outOfBound = robotStore
            .getState()
            .jointOutOfBound.reduce(
          ...
          if (!outOfBound) {
            socket.emit(
              "write",
              {
                ...
                message:
                  `M00 V0 R0 ${(
                    (robotStore.getState().angles.A0 / Math.PI) *
                    180
                  ).toFixed(3)} ` +
                  `R1 ${(
                    (robotStore.getState().angles.A1 / Math.PI) *
                    180
                  ).toFixed(3)} ` +
                  `R2 ${(
                    (robotStore.getState().angles.A2 / Math.PI) *
                    180
                  ).toFixed(3)} ` +
                  `R3 ${(
                    (robotStore.getState().angles.A3 / Math.PI) *
                    180
                  ).toFixed(3)} ` +
                  `R4 ${(
                    (robotStore.getState().angles.A4 / Math.PI) *
                    180
                  ).toFixed(3)} ` +
                  `R5 ${(
                    (robotStore.getState().angles.A5 / Math.PI) *
                    180
                  ).toFixed(3)} \r`,
              },
              (res) => {
